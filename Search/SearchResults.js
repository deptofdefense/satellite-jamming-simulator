...
const SearchResults = ({stations, searchText, onResultClick, onSearchComplete}) => {
    const results = filterResults(stations, searchText);
    if (!results) return null;

    return (
        <div className='ResultsWrapper'>
            {results.map((result, i) => <StationCard key={result.name + i} station={result} onClick={(station)=>{ onSearchComplete(); return onResultClick(station); }}/>)}
        </div>
    )
}
